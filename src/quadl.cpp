...
complex_t quadl_domain_t::quadl_1d_(complex_t (*func)(const complex_t, void*), 
    ...
    if (error>this->tol*abs(I_n)&&k<this->k_max&&error>0.0){
        k++;
        I1 = quadl_domain_t::quadl_1d_(func, args, line_1, k, I1);
        if (k>this->k_max){return I_n;}
        I2 = quadl_domain_t::quadl_1d_(func, args, line_2, k, I2);
        if (k>this->k_max){return I_n;}
        I_n = I1+I2;
    }
    return I_n;
...
complex_t quadl_domain_t::quadl_2d(complex_t (*func)(const complex_t, const complex_t, void*), 
    ...
    complex_t sum=0.0;
    const size_t N=4;
    const real_t alpha_mn[N]={1.0/3.0, 0.6, 0.2, 0.2};
    const real_t beta_mn[N]={1.0/3.0, 0.2, 0.6, 0.2};
    const real_t w_mn[N]={-27.0/48.0, 25.0/48.0, 25.0/48.0, 25.0/48.0};    
    real_t alpha, beta;
    for (size_t n=0; n<N; n++){
        alpha = triangle.v1.x+
                (triangle.v2.x-triangle.v1.x)*alpha_mn[n]+
                (triangle.v3.x-triangle.v1.x)*beta_mn[n];
        beta  = triangle.v1.y+
                (triangle.v2.y-triangle.v1.y)*alpha_mn[n]+
                (triangle.v3.y-triangle.v1.y)*beta_mn[n];
        sum+=w_mn[n]*func(alpha, beta, args);
    }
    return A*sum;
}
...
complex_t quadl_domain_t::quadl_2d_(complex_t (*func)(const complex_t, const complex_t, void*), 
    ...
    if (error>this->tol*abs(I_n)&&error>0.0){
        k++;
        I1 = quadl_domain_t::quadl_2d_(func, args, triangle_1, k, I1);
        if (k>this->k_max){return I_n;}
        I2 = quadl_domain_t::quadl_2d_(func, args, triangle_2, k, I2);
        if (k>this->k_max){return I_n;}
        I3 = quadl_domain_t::quadl_2d_(func, args, triangle_3, k, I3);
        if (k>this->k_max){return I_n;}
        I4 = quadl_domain_t::quadl_2d_(func, args, triangle_4, k, I4);
        if (k>this->k_max){return I_n;}
        I_n = I1+I2+I3+I4;
    }
    return I_n;
...
complex_t quadl_domain_t::quadl_3d_(complex_t (*func)(const complex_t, const complex_t, const complex_t, void*), 
    ...
    if (error>this->tol*abs(I_n)&&error>0.0){
        k++;
        I1 = quadl_domain_t::quadl_3d_(func, args, tetrahedron_1, k, I1);
        if (k>this->k_max){return I_n;}
        I2 = quadl_domain_t::quadl_3d_(func, args, tetrahedron_2, k, I2);
        if (k>this->k_max){return I_n;}
        I3 = quadl_domain_t::quadl_3d_(func, args, tetrahedron_3, k, I3);
        if (k>this->k_max){return I_n;}
        I4 = quadl_domain_t::quadl_3d_(func, args, tetrahedron_4, k, I4);
        if (k>this->k_max){return I_n;}
        I5 = quadl_domain_t::quadl_3d_(func, args, tetrahedron_5, k, I5);
        if (k>this->k_max){return I_n;}
        I6 = quadl_domain_t::quadl_3d_(func, args, tetrahedron_6, k, I6);
        if (k>this->k_max){return I_n;}
        I7 = quadl_domain_t::quadl_3d_(func, args, tetrahedron_7, k, I7);
        if (k>this->k_max){return I_n;}
